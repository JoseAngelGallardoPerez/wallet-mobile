...
class VerifyCodeScreenState extends State<VerifyCodeScreen> {
  ...
  void _resendRegistrationCode() async {
    ...
    request.listen((resource) {
      switch (resource.status) {
        ...
        case Status.error:
          showToast(
              context, resource.errors?.first?.message ?? 'smth went wrong');
          break;
      }
    });
  }
  ...
  void _verifyRegistrationSmsCode(UserRepository userRepository) async {
    ...
    if (widget.receiverType == CodeReceiverType.sms) {
      request =
          userRepository.checkRegistrationSmsCode(_inputController.text.trim());
    } else {
      request = userRepository
          .checkRegistrationEmailCode(_inputController.text.trim());
    }
  ...
  void _verifyResetPasswordCode(UserRepository userRepository) async {
    userRepository
        .checkResetPasswordCode(_inputController.text.trim())
        .listen((resource) {
      switch (resource.status) {
        case Status.success:
